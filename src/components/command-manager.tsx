...
export function CommandManager({ commands }: CommandManagerProps) {
  ...
  const generateDescription = (commandName: string, commandCode: string) => {
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        ...
        setDescriptions(prev => ({ ...prev, [commandName]: result.description || 'No description generated.' }));
      }
  ...
  return (
    <section>
      <h2 className="text-2xl font-bold font-headline mb-4">Command Center</h2>
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Terminal /> Bot Commands</CardTitle>
          <CardDescription>Explore the bot's commands and generate user-friendly descriptions with AI.</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <AccordionItem value={command.name} key={command.name}>
                ...
                <AccordionContent>
                  <div className="grid md:grid-cols-2 gap-4 pt-2">
                    <div>
                      <h4 className="font-semibold mb-2 text-sm">Command Code</h4>
                      <CodeBlock code={command.code} />
                    </div>
                    <div>
                      <h4 className="font-semibold mb-2 text-sm">AI Generated Description</h4>
                      <Card className="min-h-[100px] flex items-center justify-center p-4 bg-background">
                        {descriptions[command.name] ? (
                          ...
                          <p className="text-sm text-muted-foreground">Click generate to create a description.</p>
                        )}
                      </Card>
